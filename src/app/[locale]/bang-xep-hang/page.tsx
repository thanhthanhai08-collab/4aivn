      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/50 hover:bg-muted/50">
            ...
            <TableHead className="min-w-[200px] text-center">{itemType === 'tool' ? (locale === 'en' ? 'AI Tool Name' : 'Tên công cụ AI') : (locale === 'en' ? 'AI Model Name' : 'Tên model AI')}</TableHead>
            ...
            {itemType === 'tool' ? (
              ...
              <>
                ...
                <TableHead className="text-center min-w-[120px]">{locale === 'en' ? 'Context Length' : 'Độ dài ngữ cảnh'}<br/>(token)</TableHead>
                <TableHead className="text-center min-w-[100px]">{locale === 'en' ? 'Intelligence' : 'Chỉ số'}<br/>{locale === 'en' ? 'Score' : 'thông minh'}</TableHead>
                <TableHead className="text-center min-w-[140px]">{locale === 'en' ? 'Avg. Price' : 'Giá trung bình'}<br/>(USD/1M token)</TableHead>
                <TableHead className="text-center min-w-[100px]">{locale === 'en' ? 'Speed' : 'Tốc độ'}<br/>(token/s)</TableHead>
                <TableHead className="text-center min-w-[100px]">{locale === 'en' ? 'Latency' : 'Độ trễ'}<br/>(s)</TableHead>
              </>
            )}

            <TableHead className="text-center min-w-[120px]">{locale === 'en' ? 'Rating' : 'Đánh giá'}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
              ...
              <TableCell>
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex flex-col">
                    <Link href={itemType === 'model' ? { pathname: '/bang-xep-hang/[id]', params: { id: item.id } } : { pathname: '/cong-cu/[id]', params: { id: item.id } }} className="font-medium hover:underline transition-colors">
                        {item.name}
                    </Link>
       ...
       {sortedItems.length === 0 && (
        <div className="p-4 text-center text-muted-foreground">
         Không có {itemType === 'tool' ? (locale === 'en' ? 'tools' : 'công cụ') : (locale === 'en' ? 'models' : 'model')} {locale === 'en' ? 'to display.' : 'nào để hiển thị.'}
        </div>
      )}
    </div>
  );
      ...
      <div className="container py-8 md:py-12">
        <nav aria-label="Breadcrumb" className="mb-6 flex items-center text-sm font-medium hidden">
            ...
            <li className="flex items-center before:content-['/'] before:mx-2 before:text-muted-foreground/30">
                ...
                {locale === 'en' ? 'Rankings' : 'Bảng xếp hạng'}
                </span>
            </li>
            </ol>
        </nav>
        <header className="mb-8 text-center">
          ...
          <p className="mt-2 text-lg text-muted-foreground">
            {locale === 'en' ? 'Compare top AI tools and models based on performance and community ratings.' : 'So sánh các công cụ và mô hình AI hàng đầu dựa trên hiệu suất và đánh giá của cộng đồng.'}
          </p>
        </header>
