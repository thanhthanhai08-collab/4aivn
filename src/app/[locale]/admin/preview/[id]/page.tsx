...
function NewsDetailContent() {
  ...
  // Bookmark and Share functions are kept for UI consistency in preview
  ...
  const handleShare = () => {
    navigator.clipboard.writeText(window.location.href).then(() => {
      toast({
        title: tNewsDetail("copiedLink"),
        description: tNewsDetail("copiedLinkDesc"),
      });
    }).catch(err => {
      console.error('Failed to copy link: ', err);
      toast({
        title: tNewsDetail("error"),
        description: tNewsDetail("copyError"),
        variant: "destructive",
      });
    });
  };
                ...
                <div className="lg:col-span-8">
                    ...
                    <header className="mb-8">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                            ...
                            <div className="flex items-center space-x-2">
                                <Button variant="outline" onClick={handleBookmarkToggle}>
                                    ...
                                    {isBookmarked ? tNewsDetail("bookmarked") : tNewsDetail("bookmark")}
                                </Button>
                                <Button variant="outline" onClick={handleShare}>
                                    <Share2 className="mr-2 h-4 w-4" /> {tNewsDetail("share")}
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center">
                            ...
                            <span>{tNewsDetail("publishedAt", { date: format(new Date(article.publishedAt), "d MMMM, yyyy", { locale: locale === 'en' ? enUS : vi }) })}</span>
                        </div>
                        </div>
                    </header>
                    ...
                    <footer className="mt-12 pt-6 border-t">
                        {article.link && (
                            ...
                            <p className="text-sm text-muted-foreground">{tNewsDetail("source")}</p>
                    ...
                    <CardHeader>
                        ...
                        <MessageSquare className="mr-3 h-6 w-6" /> {tNewsDetail("discussion")} ({comments.length})
                        </h2>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        ...
                        <div className="text-center text-sm text-muted-foreground bg-muted/50 p-4 rounded-md">
                            <Link href="/dang-nhap" className="font-semibold text-primary hover:underline">{tNewsDetail("loginToComment")}</Link>{tNewsDetail("loginToCommentSuffix")}
                        </div>
                        )}
                        <Separator />
