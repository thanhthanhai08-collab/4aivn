...
const QuickViewItem = ({ article, locale }: { article: NewsArticle, locale: string }) => {
    ...
    <div className="flex items-center space-x-4 group">
        ...
        <div className="flex-grow">
            <Link href={`/${slug}`} className="font-semibold text-sm text-foreground hover:text-primary transition-colors line-clamp-2">{article.title}</Link>
        </div>
        <Link href={`/${slug}`} className="flex-shrink-0 w-24">
                ...
                {featuredArticle && (
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
                        <div className="group relative rounded-lg overflow-hidden shadow-lg flex flex-col md:col-span-3">
                           <Link href={`/${getLocalizedSlug(featuredArticle.slug || featuredArticle.id, locale) || featuredArticle.id}`} className="block aspect-[16/9] relative">
                                <Image
                           ...
                           <div className="p-4 bg-card flex-grow flex flex-col">
                                <h2 className="text-2xl font-bold font-headline text-foreground leading-tight flex-grow">
                                     <Link href={`/${getLocalizedSlug(featuredArticle.slug || featuredArticle.id, locale) || featuredArticle.id}`} className="hover:text-primary transition-colors">{featuredArticle.title}</Link>
                                </h2>
                        ...
                        {secondaryArticle && (
                            <div className="p-4 rounded-lg bg-card border flex flex-col justify-between md:col-span-2">
                                <div className="space-y-3">
                                     <Link href={`/${getLocalizedSlug(secondaryArticle.slug || secondaryArticle.id, locale) || secondaryArticle.id}`} className="block aspect-[16/9] relative rounded-md overflow-hidden group">
                                         <Image
                                    ...
                                    <div>
                                        <h2 className="text-xl font-bold font-headline mb-2">
                                            <Link href={`/${getLocalizedSlug(secondaryArticle.slug || secondaryArticle.id, locale) || secondaryArticle.id}`} className="hover:text-primary">{secondaryArticle.title}</Link>
                                        </h2>
                                ...
                                <Button asChild variant="link" className="p-0 self-start mt-4">
                                    <Link href={`/${getLocalizedSlug(secondaryArticle.slug || secondaryArticle.id, locale) || secondaryArticle.id}`}>{t("readMore")} &rarr;</Link>
                                </Button>
                            </div>
                        )}
                    </div>
