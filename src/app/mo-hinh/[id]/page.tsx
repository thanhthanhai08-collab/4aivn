...
function ModelDetailContent({ id }: { id: string }) {
  ...
  if (['openai-o3', 'grok-4', 'openai-o3-pro', 'gemini-2.5-pro', 'gpt-5-mini-high', 'gpt-5-high', 'qwen3-235b-reasoning', 'gpt-5-medium', 'deepseek-r1-jan25', 'deepseek-v3.1-reasoning', 'gemini-2.5-flash-reasoning', 'gpt-5-mini-medium', 'gpt-5-low', 'gpt-oss-120b-high', 'gpt-5-nano', 'claude-4-opus-thinking', 'claude-4.1-opus-thinking', 'claude-4.5-sonnet-thinking', 'grok-3-mini-reasoning-high', 'llama-nemotron-ultra-reasoning', 'llama-4-maverick', 'claude-4.1-opus', 'gpt-oss-20b-high', 'gpt-5-codex-high', 'qwenq-32b'].includes(model.id)) {
    const isO3 = model.id === 'openai-o3';
    const isGpt5High = model.id === 'gpt-5-high';
    const isO4MiniHigh = model.id === 'gpt-5-mini-high';
                    ...
                    <CardContent className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 text-sm">
                        ...
                        <div className="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p className="text-muted-foreground">{['claude-4.1-opus', 'claude-4.1-opus-thinking', 'llama-nemotron-ultra-reasoning', 'llama-4-maverick', 'gpt-5-codex-high'].includes(model.id) ? 'Có' : (isQwen3 || isDeepseekR1 || model.id === 'deepseek-v3.1-reasoning' || isClaude4OpusThinking || isClaude45SonnetThinking ? 'Không' : 'Có')}</p>
                            </div>
                        </div>
                        <div className="flex items-start space-x-3">
                        ...
                        <div className="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p className="text-muted-foreground">{model.id === 'openai-o3' ? '20/12/2024' : model.id === 'gpt-5-mini-high' ? '16/04/2025' : (model.id === 'gpt-5-high' || model.id === 'gpt-5-medium' || model.id === 'gpt-5-mini-medium' || model.id === 'gpt-5-low' || model.id === 'gpt-5-nano') ? '07/08/2025' : model.id === 'qwen3-235b-reasoning' ? '25/07/2025' : model.id === 'deepseek-r1-jan25' ? '28/05/2025' : model.id === 'deepseek-v3.1-reasoning' ? '21/08/2025' : model.id === 'gemini-2.5-flash-reasoning' ? '17/06/2025' : (model.id === 'gpt-oss-120b-high' || model.id === 'gpt-oss-20b-high' || model.id === 'claude-4.1-opus') ? '05/08/2025' : (model.id === 'claude-4-opus-thinking' || model.id === 'claude-4.5-sonnet-thinking') ? '22/05/2025' : model.id === 'claude-4.1-opus-thinking' ? '05/08/2025' : model.id === 'grok-3-mini-reasoning-high' ? '17/02/2025' : model.id === 'llama-nemotron-ultra-reasoning' ? '07/04/2025' : model.id === 'llama-4-maverick' ? '05/04/2025' : model.id === 'gpt-5-codex-high' ? '15/09/2025' : model.id === 'qwenq-32b' ? '06/03/2025' : '09/07/2025'}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                
                 {['openai-o3', 'grok-4', 'gemini-2.5-pro', 'gpt-5-mini-high', 'openai-o3-pro', 'gpt-5-high', 'qwen3-235b-reasoning', 'gpt-5-medium', 'deepseek-r1-jan25', 'deepseek-v3.1-reasoning', 'gemini-2.5-flash-reasoning', 'gpt-5-mini-medium', 'gpt-5-low', 'gpt-oss-120b-high', 'gpt-5-nano', 'claude-4-opus-thinking', 'claude-4.1-opus-thinking', 'claude-4.5-sonnet-thinking', 'grok-3-mini-reasoning-high', 'llama-nemotron-ultra-reasoning', 'llama-4-maverick', 'claude-4.1-opus', 'gpt-oss-20b-high', 'gpt-5-codex-high', 'qwenq-32b'].includes(model.id) && (
                    <section>
