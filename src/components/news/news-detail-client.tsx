...
const renderContent = (article: NewsArticle) => {
  ...
  return parts.map((part, index) => {
    ...
    if (imageMatch) {
      ...
      return (
        <figure key={`${index}-img`} className="not-prose w-full my-6 lg:my-8 flex flex-col items-center justify-center">
          {/* Container cho ảnh */}
...
export function NewsDetailClient({ article, latestNews, relatedNews }: Props) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = getComments(article.id, (fetchedComments) => {
